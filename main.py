...
def moves(s):
    ...
    action = []
    value_stack = []
    temp_updated_result = 0

    while True:
        print('firstAt line185 ', value_stack)
        try:
            ...
            # Check if the parsing table contains the entry for the current state and input
            #if (state_id, ('action', current_input)) not in parsing_table.index:
             #   raise SyntaxError(f"Unexpected input '{current_input}' at state {state_id}")
            ...
            if a == 'accept':
                ...
                if len(value_stack) == 1:
                    print('Expression result:', value_stack[0])
                break
            ...
            if a.startswith('s'):
                ...
                # Ensure we don't run out of elements in all_scope and input_
                if current_input == 'N':
                    if not all_scope:
                        raise SyntaxError("Unexpected end of input during shift action.")
                    if type(all_scope[0]) == int:
                        value_stack.append(int(all_scope.pop(0)))
                    else:
                        all_scope.pop(0)
                input_.pop(0)
            ...
            # Handle reduce actions
                ...
                # Check if the stack and value stack have enough elements to perform reduction
                if len(value_stack) < num_symbols_rhs or len(stack) < num_symbols_rhs:
                    print('DEBUGPOINT line228, possible potential issue')
                    #raise SyntaxError(f"Insufficient stack size for reduction: rule {rule}")

                reduced_values = value_stack[-num_symbols_rhs:]
                #del value_stack[-num_symbols_rhs:]

                # Evaluate reduction and handle specific operations
                if rule.lhs == '`E':
                    if rule.rhs == ['`E', '+', '`T']:
                        temp_updated_result = reduced_values[0] + reduced_values[1]
                        value_stack.append(reduced_values[0] + reduced_values[1])
                    elif rule.rhs == ['`E', '-', '`T'] and len(reduced_values) > 1:
                        temp_updated_result = reduced_values[0] + reduced_values[1]
                        value_stack.append(reduced_values[0] - reduced_values[1])
                elif rule.lhs == '`T':
                    if rule.rhs == ['`T', '*', 'N'] and len(reduced_values) > 1:
                        temp_updated_result = reduced_values[0] + reduced_values[1]
                        value_stack.append(reduced_values[0] * reduced_values[1])
                    elif rule.rhs == ['`T', '/', 'N']:
                        if reduced_values[1] == 0:
                            raise ZeroDivisionError("Division by zero in expression")
                        value_stack.append(reduced_values[0] / reduced_values[1])
                elif rule.lhs == 'N':
                    value_stack.append(reduced_values[0])
                    temp_updated_result = reduced_values[0]

                # Update stack by popping elements and adding new goto state
                for _ in range(num_symbols_rhs):
                    stack.pop()
                
                goto_state_id = parsing_table.loc[(stack[-1][1]), ('goto', rule.lhs)]
                stack.append((rule.lhs, goto_state_id))
                action[-1] = f'{a} goto: {goto_state_id} {rule}'
            ...
            print('Exception point, line 266')
...
'''def moves(s):
    ...
    while True:
        print('value_stack ele 185Line', value_stack)
        ...
        if a == 'accept':
            print('Driver: accept')
            if len(value_stack) == 1:
                print('value_stack ele 193Line', value_stack)
            break
        ...
        if a.startswith('s'):
            ...
            print('input[0] char or token DEBUG', input_[0])
            if input_[0] == 'N':
                value_stack.append(int(all_scope[0]))
            input_.pop(0)
            all_scope.pop(0)
        
        # Reduce
        elif a.startswith('r'):
            rule_index = int(''.join(a[1:]))
            r = Rule.augmented[rule_index]
            num_symbols_rhs = len(r.rhs)
            print('value_stack ele 211Line', value_stack)

            # Perform the reduction
            reduced_value = value_stack[-num_symbols_rhs:]
            #del value_stack[-num_symbols_rhs:]

            # Evaluate the reduction if there are values to evaluate
            #reduced_value.append(5) #hard
            #value_stack.append(5) #hard
            if num_symbols_rhs > 1:
                if r.lhs == '`E':
                    if r.rhs == ['`E', '+', '`T']:
                        # Check if reduced_value has at least 2 elements
                        if len(reduced_value) >= 2:
                            tempp = reduced_value[0] + reduced_value[1]
                            value_stack.pop()
                            value_stack.append(tempp)
                    elif r.rhs == ['`E', '-', '`T']:
                        # Check if reduced_value has at least 2 elements
                        if len(reduced_value) >= 2:
                            tempp = reduced_value[0] - reduced_value[1]
                    ...
                    if r.rhs == ['`T', '*', 'N']:
                        ...
                        if len(reduced_value) >= 2:
                            ...
                            value_stack.append(tempp)
                        #else:
                            #print('Insufficient values for multiplication.')
                    elif r.rhs == ['`T', '/', 'N']:
                        # Check if reduced_value has at least 2 elements
                        if len(reduced_value) >= 2:
                            if reduced_value[1] == 0:
                                raise ZeroDivisionError("Division by zero in expression")
                            value_stack.append(reduced_value[0] / reduced_value[1])
                        else:
                            print('Insufficient values for division.')
            else:
                pass
                print('value_stack ele 247Line', value_stack)
                # Handle case where num_symbols_rhs <= 1
                #print('Unexpected num_symbols_rhs value for rule:', r)

            # Update stack with GOTO information
            for _ in range(num_symbols_rhs):
                stack.pop()
                print('value_stack ele 254Line', value_stack)
...
if __name__ == '__main__':
    ...
    #print(g4, end='\n------grammar------\n\n')
    S = SyntaxAnalyzer()
    expressionBottom_up_p = "100-12*12"
...
###############################################
    ...
    #lexemes, tokens = S.lexer("7+5*3")
    #once_pop_tokens = tokens[:-1]
